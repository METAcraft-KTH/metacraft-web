...
<div class="relative mb-10 sm:mb-12 sm:flex flex-row flex-justify-center flex-items-center">
  {#if left}
    <img src={imgSrc} alt={imgAlt} class="relative left--2 m-2 w-40% float-left sm:float-none sm:w-180px md:w-240px xl:scale-120 xl:left--12 b-white b-4 b-solid shadow-lg rotate--2 hover:rotate--4 transition-all sepia-40" />
  {/if}
  {#if !left}
